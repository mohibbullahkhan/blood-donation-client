...
// --- Helper Components (StatusBadge, DetailItem, ActionButton) ---
const StatusBadge = ({ status, className = "" }) => {
  ...
  return (
    <span
      className={`flex items-center px-2 py-0.5 text-xs leading-5 font-semibold rounded-full capitalize border ${colorClass} ${className}`}
    >
      {icon}
      {text}
    </span>
...
const AllBloodDonationRequest = () => {
  const { isAdmin, isVolunteer, canManageRequests } = useAuth();
  ...
  //  State for the dedicated donor assignment modal
  ...
  // Function triggered by the AssignDonorModal
  const confirmAssignDonor = async (requestId, donorName, donorEmail) => {
    if (!isAdmin) {
    ...
    if (!donorName || !donorEmail) {
      alert("Donor name and email are required.");
      return;
    }
  ...
  // ---  Modal Component: AssignDonorModal ---
  const AssignDonorModal = () => {
    ...
    const handleSubmit = (e) => {
      e.preventDefault();
      confirmAssignDonor(selectedRequest._id, donorName, donorEmail);
    };
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-gray-900 bg-opacity-80 flex justify-center items-center p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
          <form onSubmit={handleSubmit}>
            <div className="p-6">
              <div className="flex justify-between items-center border-b pb-4 mb-4 border-blue-500">
                <h3 className="text-xl font-bold text-blue-600 flex items-center">
                  <UserPlus className="w-5 h-5 mr-2" /> Assign Donor
                </h3>
                <button
                  type="button"
              ...
              <p className="mb-4 text-sm text-gray-700">
                Assign a donor to the request for **
                {selectedRequest.recipientName}** (Blood Group:{" "}
              ...
              <div className="flex justify-end space-x-3 mt-6">
                <button
                  type="button"
                  onClick={() => setIsAssignModalOpen(false)}
                  className="px-4 py-2 text-sm font-semibold text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 transition shadow-sm"
                >
                  Cancel
                </button>
                <ActionButton
          ...
          <div className="p-6">
            ...
            <div className="pt-4 border-t border-gray-200">
              ...
              <div className="flex flex-wrap gap-3">
                {/* 1. VOLUNTEER AND ADMIN ONLY: Assign Donor */}
                {isAdmin && isPending && (
                  <ActionButton
                    // UPDATED: Calls the handler which opens
                    onClick={() => handleAssignDonor(selectedRequest)}
                    Icon={UserPlus}
                ...
                {/*  Cancel Request (if pending or in progress) */}
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
      ...
      {/*Assign Donor Modal */}
      {isAssignModalOpen && <AssignDonorModal />}
    </div>
  );
};
